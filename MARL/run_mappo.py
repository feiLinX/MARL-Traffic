...
def train(args):
    ...
    env_eval.config['traffic_density'] = config.getint('ENV_CONFIG', 'traffic_density_eval')
    ...
    eval_std = []
    episodes = []
    print(f"density{traffic_density}to{env_eval.config['traffic_density']}")
    print(f"save file: rewardPPO_bsz{BATCH_SIZE}_density{traffic_density}to{env_eval.config['traffic_density']}_gamma{reward_gamma}.pdf")
    while mappo.n_episodes < MAX_EPISODES:
        ...
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            ...
            print("Episode %d, Average Reward %.2f, Std %.2f" % (mappo.n_episodes + 1, rewards_mu, rewards_std))
    ...
    plt.legend(["MAPPO"])
    plt.savefig(f"rewardPPO_bsz{BATCH_SIZE}_density{traffic_density}to{env_eval.config['traffic_density']}_gamma{reward_gamma}_1to2.pdf")
